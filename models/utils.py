...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.fc1 = BinaryLinear_adapscaling(in_features, hidden_features)
        self.act = act_layer()
        self.fc2 = BinaryLinear_adapscaling(hidden_features, out_features)
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # q = q * self.scale
        attn = (binary_q @ binary_k.transpose(-2, -1)) * self.scale
        ...
        if mask is not None:
            ...
            probs = self.attn_quant_layer(attn).float().detach() - attn.softmax(-1).detach() + attn.softmax(-1)
        else:
            probs = self.attn_quant_layer(attn).float().detach() - attn.softmax(-1).detach() + attn.softmax(-1)

        probs = self.attn_drop(attn)

        x = (probs @ binary_v).transpose(1, 2).reshape(B_, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm):
        ...
        self.input_resolution = input_resolution
        self.dim = dim
        self.reduction = BinaryLinear_adapscaling(4 * dim, 2 * dim, bias=False)
